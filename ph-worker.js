self.onmessage = (ev) => {
    const { id, points, maxdim } = ev.data;
    const result = computeVRPersistence(points, Math.min(1, maxdim ?? 1));
    self.postMessage({ id, ...result });
};

function computeVRPersistence(pts, maxdim) {
    const n = pts.length;
    ...
    let maxVal = Math.max(1e-6, maxF) * 1.05;
    return { H0, H1, maxVal };
}
