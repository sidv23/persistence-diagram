import { computeKnnWeights, radiusAt } from './weighted-math.js';
...
let kVal = Number(kInput.value) || 10;
let pVal = Number(pInput.value) || 2;

// weights w_i (normalized)
let weights = new Float32Array(points.length);

// ---------- Worker ----------
const worker = new Worker('./ph-worker2.js', { type: 'module' });
...
const ro = new ResizeObserver(() => {
  resizeAll();
  // weights are in normalized units, independent of resize
  drawAll();
});
ro.observe(ptsBox);
ro.observe(pdBox);

// slider input (draw only)
...
// k, p changes affect weights + persistence
kInput.addEventListener('change', () => {
  kVal = sanitizeK(Number(kInput.value));
  kInput.value = String(kVal);
  recomputeWeights();
  drawAll();
  requestCompute();
});
...
pInput.addEventListener('change', () => {
  pVal = sanitizeP(Number(pInput.value));
  pInput.value = String(pVal);
  drawAll();
  requestCompute();
});
...
function recomputeWeights() {
  ...
  weights = computeKnnWeights(payload, kVal);
}
...
function drawPointCloud() {
  ...
  // Weighted balls:
  // r_i(t) = max(t^p - w_i^p, 0)^(1/p)
  // t slider is in pixels => convert to normalized, compute r_i in normalized, then convert back to px.
  if (tPx > 0) {
...
function drawPersistenceDiagram() {
  ...
  // dotted reflected lines at threshold t (same t as the ball visualization)
  const tNorm = (sizePx > 0) ? (tPx / sizePx) : 0;
